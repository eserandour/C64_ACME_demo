...
; Variables internes à drawCircleFill (en plus de celles de drawCircle) :
rowY_lo     = $6d        ; ligne horizontale à remplir (signée, avant clipping)
rowY_hi     = $6e
spanL_lo    = $6f        ; bornes gauche/droite du segment à remplir
spanL_hi    = $70
spanR_lo    = $71
spanR_hi    = $72
...
start:
        ...
        ; drawCircleFill(250, 50, 60, 13) : un disque plein vert clair
        lda #<250
        sta paramX_lo
        lda #>250
        sta paramX_hi
...
; ================================================================
; 10) drawCircleFill(xc, yc, r, couleur)
; Trace un disque PLEIN. Même algorithme de décision que drawCircle,
; mais au lieu de tracer 8 points isolés à chaque pas, on remplit 4
; LIGNES HORIZONTALES (une paire de rangées symétriques par rapport
; au centre, pour les deux rôles cx/cy) via fillRowFast — la même
; sous-routine rapide déjà utilisée par drawRectFill et
; drawPolygonFill. C'est l'équivalent du _fmemset de ton graphics.h,
; adapté à la contrainte "bloc par bloc" déjà rencontrée.
; ================================================================
drawCircleFill:
        lda paramX_lo
        sta xc_lo
...
; ----------------------------------------------------------------
; fillCirclePoints : remplit les 4 lignes horizontales correspondant
; à la position courante (cx,cy) de l'algorithme :
;   ligne yc+cy, de xc-cx à xc+cx
;   ligne yc-cy, de xc-cx à xc+cx
;   ligne yc+cx, de xc-cy à xc+cy
;   ligne yc-cx, de xc-cy à xc+cy
; ----------------------------------------------------------------
fillCirclePoints:
        lda cx
...
; ----------------------------------------------------------------
; fillCircleRow : remplit le segment [spanL,spanR] de la ligne rowY,
; avec clipping complet (ligne hors écran ignorée, segment débordant
; ramené aux bornes 0-319). Réutilise fillRowFast (drawRectFill).
; ----------------------------------------------------------------
fillCircleRow:
        lda rowY_hi
        bmi fcr_skip
